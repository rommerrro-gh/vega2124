...
Реализовать сквозной путь `MAX Update → report → issue → candidates → join/create → статус → уведомление → приемка`, затем пройти AC-01…AC-18. Перед приёмкой нужно отдельно проверить сборку, Compose и полный пользовательский сценарий.
