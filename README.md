...
- `ReportService` сохраняет обращение, ищет кандидатов по дому, статусу и времени, создаёт черновик заявки при низком score и пишет аудит. Для внешнего вызова `POST /v1/reports` ещё нужна аутентификация: `/v1/**` закрыт до JWT.
